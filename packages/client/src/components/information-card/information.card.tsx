    ...
    <div class="m-0 p-8 box-border dark:text-white">
      ...
      >
        <a href={generateRoutingLink(location.params.lang, props.goTo)}>
          <div class="hover:shadow-md hover:scale-105 m-3 p-5 h-[18rem] w-[21rem] grid grid-rows-4 rounded-xl shadow-sm transition-all card-light dark:card-dark">
            <h1 class="font-bold text-center text-2xl">{props.name}</h1>
            <h2 class="text-center text-xl">
              <i class={props.icon}></i>
            </h2>
            <h2 class="font-semibold text-lg">{props.description}</h2>
          </div>
        </a>
      </div>
    </div>
